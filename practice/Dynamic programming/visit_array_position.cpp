...
{
public:
    long long solve(int ind, vector<int> &nums, int x, int parity, vector<vector<long long>> &dp)
    {
        if (ind == nums.size())
            return 0;
        if (dp[ind][parity] != -1)
            return dp[ind][parity];
        long long ntake = solve(ind + 1, nums, x, parity, dp);
        long long take = 0;
        if (nums[ind] % 2 == parity)
            take = nums[ind] + solve(ind + 1, nums, x, parity, dp);
        else
            take = nums[ind] - x + solve(ind + 1, nums, x, nums[ind] % 2, dp);
    ...
    long long maxScore(vector<int> &nums, int x)
    {
        ...
        int parity = nums[0] % 2;
        // return nums[0] + solve(1,nums,x,parity,dp);
